...
__author__ = "Johannes Imort, Marco Berzborn"
__version__ = "0.1.0"
...
def estimate_noise_energy(
    ...
    energy_data = preprocess_rir(
        data, is_energy=is_energy, time_shift=False, channel_independent=False)[0]
...
def preprocess_rir(data, is_energy=False, time_shift=False,
                   channel_independent=False):
    """ Preprocess the room impulse response for further processing:
        - square data
        - remove silence at the beginning, if necessary
        - the time shift can be done channel-independently or not.
    ...
    channel_independent : boolean
        Defines, if the time shift is done channel-independently or not.
    ...
    if time_shift:
        ...
        if channel_independent and not n_channels == 1: # Shift each channel indep.
            for idx_channel in range(0, n_channels):
...
def smooth_rir(data, sampling_rate, smooth_block_length=0.075):
    """ Smoothes the RIR by averaging the data in an specified interval.
...
def energy_decay_curve_truncation(
    data, sampling_rate, freq='broadband', noise_level='auto', is_energy=False,
    time_shift=True, channel_independent=False, normalize=True, plot=False):
    ...
    intersection_time = intersection_time_lundeby(
        energy_data, sampling_rate=sampling_rate, freq=freq,
        noise_level=noise_level, is_energy=True, time_shift=False,
        channel_independent=False, plot=False)[0]
    ...
    if normalize: # Normalize the EDC...
        if not channel_independent: # ...by the first element of each channel.
            energy_decay_curve = (energy_decay_curve.T /
                                  energy_decay_curve[..., 0]).T
        else: # ...by the maximum first element of each channel.
...
def energy_decay_curve_lundeby(
    ...
    energy_data, n_channels, data_shape = preprocess_rir(
        ...
        channel_independent=channel_independent)
    n_samples = energy_data.shape[-1]
    intersection_time, late_reveberation_time, noise_estimation = intersection_time_lundeby(
        energy_data, sampling_rate=sampling_rate, freq=freq,
        noise_level=noise_level, is_energy=True, time_shift=False,
    ...
    for idx_channel in range(0, n_channels):
        intersection_time_idx = np.argmin(np.abs(time_vector
                                                 - intersection_time[idx_channel]))
        p_square_at_intersection = noise_estimation[idx_channel]

        # Calculate correction term according to DIN EN ISO 3382 # TO-DO: check reference!
...
def energy_decay_curve_chu(
    ...
    References
    ----------
    ..[2]   W. T. Chu. “Comparison of reverberation measurements using Schroeder’s
    ...
    subtracted = subtract_noise_from_squared_rir(energy_data,
...
def energy_decay_curve_chu_lundeby(
    ...
    for idx_channel in range(0, n_channels):
        ...
        energy_decay_curve[idx_channel,:intersection_time_idx] = ra.schroeder_integration(
            subtraction[idx_channel,:intersection_time_idx], is_energy=True)
        energy_decay_curve[idx_channel] += correction
...
def intersection_time_lundeby(
    data, sampling_rate, freq='broadband', noise_level='auto', is_energy=False,
    time_shift=False, channel_independent=False, plot=False):
    ...
    energy_data, n_channels, data_shape = preprocess_rir(data,
        is_energy=is_energy, time_shift=time_shift,
        channel_independent=channel_independent)
    ...
    for idx_channel in range(0, n_channels):
        ...
        try:
            ...
            raise Exception('Regression failed: Low SNR. Estimation terminated.')
        ...
        if slope[1] == 0 or np.any(np.isnan(slope)):
            raise Exception(
                'Regression failed: T would be Inf, setting to 0. Estimation terminated.')
        ...
        preliminary_crossing_point = ((10*np.log10(noise_estimation[idx_channel])
                                      - slope[0])
                                      / slope[1])

        # (5) NEW LOCAL TIME INTERVAL LENGTH
        n_blocks_in_decay = (np.diff(10*np.log10(np.take(
            time_window_data_current_channel,[start_idx, stop_idx])))
        ...
        time_window_data_current_channel, time_vector_window_current_channel, time_vector_current_channel = smooth_rir(
        ...
        while(True):
            ...
            slope = np.linalg.lstsq(regression_matrix.T,
                (10*np.log10(time_window_data_current_channel[start_idx_loop:stop_idx_loop])),
                rcond=None)[0]
            ...
            if loop_counter > 30:
                ...
                warnings.warn("Lundeby algorithm was terminated after 30 iterations.")
                break
    ...
    if plot:
        ...
        plt.plot(time_vector_window_current_channel, 10*np.log10(
            time_window_data_current_channel))
        plt.xlabel('Time [s]')
