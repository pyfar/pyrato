...
def test_sti_snr_value_error():
    ...
    """
    sig = pf.Signal(np.zeros(70560), 44100)
...
def test_sti_warn_length():
    ...
    """
    sig = pf.Signal(np.ones((31072)), 44100)
...
def test_sti_1D_shape():
    ...
    """
    shape_expected = (2,)
    sig = pf.Signal(np.ones((2,70560)), 44100)
...
def test_sti_2D_shape():
    ...
    """
    shape_expected = (2,2)
    sig = pf.Signal(np.ones((2,2,70560)), 44100)
...
def test_mtf_short_signal_error():
    ...
    """
    sig = pf.Signal(np.ones(44100), 44100)  # 1 second
    snr = np.ones(7) * 30
    match = "Input signal must be at least 1.6 seconds long"
    with pytest.raises(ValueError, match=match):
        modulation_transfer_function(
