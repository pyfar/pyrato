...
import pyfar as pf
import pyrato as ra
from pyrato.parameters import clarity_from_energy_balance
...
def test_clarity_accepts_timedata_and_returns_correct_type():
    ...
    result = clarity_from_energy_balance(edc, early_time_limit=4)  # 4 ms
...
def test_clarity_rejects_non_timedata_input():
    ...
    with pytest.raises(TypeError, match=re.escape(expected_error_message)):
        clarity_from_energy_balance(invalid_input)
...
def test_clarity_rejects_non_numeric_early_time_limit():
    ...
    with pytest.raises(TypeError, match=re.escape(expected_error_message)):
        clarity_from_energy_balance(edc, invalid_time_limit)
...
#     with pytest.raises(ValueError, match=re.escape(expected_error_message)):
#         clarity_from_energy_balance(edc, early_time_limit=-1)

#     # Test time limit beyond signal length
#     with pytest.raises(ValueError, match=re.escape(expected_error_message)):
#         clarity_from_energy_balance(edc, early_time_limit=200000)

# def test_clarity_preserves_multichannel_shape():
#     energy = np.ones((2,2,10)) / (1+np.arange(10))
#     edc = make_edc_from_energy(energy, 10)
#     output = clarity_from_energy_balance(edc, early_time_limit=80)
...
#     result = clarity_from_energy_balance(edc)
...
#     result = clarity_from_energy_balance(edc, early_time_limit=2)
...
#     result = clarity_from_energy_balance(edc, early_time_limit=early_cutoff)
...
#     result = clarity_from_energy_balance(edc, early_time_limit=80)
